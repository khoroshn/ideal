@section('plugins_css')
    <link rel="stylesheet" href="{{asset(env('THEME'))}}/css/photoswipe.min.css">
    <link rel="stylesheet" href="{{asset(env('THEME'))}}/css/photoswipe-default-skin/default-skin.min.css">
@show

      <div class="row">
        <div class="col-12 d-block d-md-none">
          <div class="title"><span>{{$content[0]->name}}</span></div>
        </div>
        <div class="col-xl-4 col-lg-5 col-md-6">
          <div class="swiper-container border rounded mb-2" id="detail-slider">
            <div class="swiper-wrapper">
                @foreach($content[0]->images as $image)
                    <div class="swiper-slide"><img src="/uploads/{{$image->filename}}" class="w-100" data-width="1025" data-height="1400" alt="Product"></div>
                @endforeach
            </div>
          ...
          <div class="swiper-container detail-gallery mb-2" id="detail-gallery">
            <div class="swiper-wrapper">
                @foreach($content[0]->images as $image)
                    <div class="swiper-slide"><a href="#"><img src="/uploads/{{$image->filename}}" alt="Product" class="img-thumbnail"></a></div>
                @endforeach
            </div>
          ...
          <table class="table table-detail">
            <tbody>
              <tr class="d-none d-md-table-row">
                <td class="border-top-0" colspan="2"><h5>{{$content[0]->name}}</h5></td>
              </tr>
              <tr>
                <td>Price</td>
                <td>
              ...
              @if(count($content[0]->groups)>0)
              <tr>
                <td>Availability</td>
                <td>
                    @foreach($content[0]->groups as $group)
                  ...
                  <div class="row">
                    <div class="col-5 col-sm-3 col-md-6 col-lg-3 col-xl-2">
                      <select name="size" class="custom-select custom-select-sm">
                          ...
                          @foreach($content[0]->sizes as $size)
                        <option value="{{$size->id}}">{{$size->name_inter}}</option>
                          @endforeach
                      </select>
                    </div>
                  </div>
              ...
              <tr class="d-none d-md-table-row">
                ...
                <td>
                    @foreach($content[0]->colors as $color)
                  <div class="custom-control custom-radio">
                    <input type="radio" id="option{{$color->id}}" name="quickview-option" class="custom-control-input">
                    <label class="custom-control-label" for="option{{$color->id}}">{{$color->name}}</label>
                  </div>
                    @endforeach
                </td>
              </tr>
                ...
                <td>
                  <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                   {{-- <button  type="button" class="btn btn-theme">ADD TO CART</button>--}}
                    <a href="/cart/add/{{$content[0]->id}}" type="button" class="btn btn-theme">ADD TO CART</a>
